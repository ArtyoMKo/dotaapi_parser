...
class TestClassForHelperMethods(unittest.TestCase):
    ...
    def test_request_template(self):
        ...
        assert (
            request_template(
                "https://api.opendota.com/api/playersByRank", mock_requests
            )["mocked_data"]
            is self.test_data["mocked_data"]
        )

    def test_request_template_neg_connection_error(self):
        mock_requests = mock_get_request(
    ...
    def test_compute_kda_neg(self):
        kda = compute_kda(
            self.test_data["kills"],
            self.test_data["deaths"],
            self.test_data["assists_neg"],
        )
...
class TestLogs(unittest.TestCase):
    ...
    def test_request_template_connection_error_log(self):
        ...
        assert (
            ...
            == self.messages["messages"]["response"]["connection_error"]
        )
    ...
    def test_request_template_timeout_log(self):
        ...
        assert (
            captured.records[0].getMessage()
            == self.messages["messages"]["response"]["timeout"]
        )

    def test_request_template_missing_schema_log(self):
        with self.assertLogs() as captured:
            mock_template_request_for_log("", requests.exceptions.MissingSchema)
        assert (
            captured.records[0].getMessage()
            == self.messages["messages"]["response"]["missing_schema"]
        )

    def test_request_template_unknown_error_log(self):
        with self.assertLogs() as captured:
            mock_template_request_for_log(
                "https://api.opendota.com/api/playersByRank", "Unknown exception"
            )
        assert (
            captured.records[0].getMessage()
            == self.messages["messages"]["response"]["unknown_exception"]
        )
    ...
    def test_console_parser_neg_log(self):
        ...
        assert (
            captured.records[0].getMessage()
            == self.messages["messages"]["console"]["negative"]
        )
    ...
    def test_console_parser_log(self):
        ...
        assert (
            captured.records[0].getMessage()
            == self.messages["messages"]["console"]["hug"]
        )
    ...
    def test_read_json_log(self):
        with raises(FileNotFoundError):
            with self.assertLogs() as captured:
                read_json("invalid_json.json")
        assert (
            self.messages["messages"]["read_json"]["not_found"]
