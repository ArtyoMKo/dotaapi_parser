...
class TestClassForDotaApi(unittest.TestCase):
    ...
    def test_get_recent_matches_for_player_response(self):
        ...
        assert result == self.test_data['responses']['recent_matches']['assert_recent_matches_first_match']
    ...
    def test_get_player_score_for_matches_keys(self):
        ...
        assert result == self.test_data['responses']['player_score_for_matches']['assert_player_matches_data_keys']

    def test_get_player_score_for_matches_data_length(self):
        result = len(mock_player_score_for_matches_response()['kills'])
        assert result == self.test_data['responses']['player_score_for_matches']['assert_kills_list_length']

    def test_construct_player_data_keys(self):
        result = list(mock_construct_player_data(self.test_data['player_data']['valid_no_zero_player_matches_data'])
                      .keys())
        assert result == self.test_data['player_data']['assert_keys']

    def test_construct_player_data_fields(self):
        data = mock_construct_player_data(self.test_data['player_data']['valid_no_zero_player_matches_data'])
        result = [
            data['max_KDA'],
            data['max_KP']
        ]
    ...
    def test_construct_player_data_fields_neg(self):
        data = mock_construct_player_data(self.test_data['player_data']['empty_player_matches_data'])
        result = [
            data['max_KDA'],
            data['max_KP']
        ]
